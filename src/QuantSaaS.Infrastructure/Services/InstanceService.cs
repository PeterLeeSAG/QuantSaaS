...
public sealed class InstanceService : IInstanceService
{
    ...
    public InstanceService(DbConnectionFactory db) => _db = db;
        ...
        Guid instanceId, CancellationToken ct = default)
    {
        ...
        var trades = await conn.QueryAsync<dynamic>("""
            SELECT symbol, asset_class, action, filled_qty, filled_price, fee, status, filled_at
            FROM trade_records
        ...
        var settlements = await conn.QueryAsync<dynamic>("""
            ...
            WHERE instance_id = @Id AND is_settled = FALSE
