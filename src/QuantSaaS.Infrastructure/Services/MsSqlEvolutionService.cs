...
public sealed class MsSqlEvolutionService : IEvolutionService
{
    ...
    public async Task<LabDto> GetLabAsync(Guid userId, CancellationToken ct = default)
    {
        ...
        var taskRows = await conn.QueryAsync<dynamic>("""
            ...
            FROM dbo.evolution_tasks et
            JOIN dbo.strategy_templates st ON st.id = et.template_id
            JOIN dbo.strategy_instances si ON si.template_id = st.id AND si.user_id = @UserId
        ...
        var geneRows = await conn.QueryAsync<dynamic>("""
            ...
            FROM dbo.gene_records gr
            JOIN dbo.strategy_templates st ON st.id = gr.template_id
            JOIN dbo.strategy_instances si ON si.template_id = st.id AND si.user_id = @UserId
