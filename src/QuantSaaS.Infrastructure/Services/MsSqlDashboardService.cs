...
public sealed class MsSqlDashboardService : IDashboardService
{
    ...
    public async Task<DashboardSummaryDto> GetSummaryAsync(Guid userId, CancellationToken ct = default)
    {
        ...
        var availableFunds = await conn.ExecuteScalarAsync<decimal>("""
            ...
            FROM dbo.portfolio_snapshots ps
            JOIN dbo.strategy_instances si ON si.id = ps.instance_id
            WHERE si.user_id = @UserId
            """, new { UserId = userId });
        ...
        // Fallback: if no snapshots yet, derive a flat curve from current equity.
        if (curve.Count == 0)
        {
